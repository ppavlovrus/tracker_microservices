...
class TestService:
    ...
    async def create_record(self, record: Record) -> Record:
        ...
        return Record(
            record_type=saved_model.record_type, record_owner=saved_model.record_owner, amount=saved_model.amount
        )
    ...
    async def get_record_by_id(self, record_id: int) -> Record | None:
        ...
        if model is None:
            return None
        return Record(record_type=model.record_type, record_owner=model.record_owner, amount=model.amount)

    async def get_all_records(self) -> list[Record]:
        models = await self.repository.get_all()
        return [Record(record_type=m.record_type, record_owner=m.record_owner, amount=m.amount) for m in models]
