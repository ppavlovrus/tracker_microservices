        ...
        async with self.pool.acquire() as conn:
            try:
                new_id = await conn.fetchval(
                    "INSERT INTO main_records (record_type,record_owner,amount) VALUES ($1, $2, $3) RETURNING id",
                    model_record.record_type,
                    model_record.record_owner,
                    model_record.amount,
                )
                return ModelRecord(
                    id=new_id,
                    record_type=model_record.record_type,
                    record_owner=model_record.record_owner,
                    amount=model_record.amount,
                )
        ...
        try:
            async with self.pool.acquire() as conn:
                updated_record = await conn.fetchrow(
                    "UPDATE main_records SET amount = $1, record_type = $2, record_owner = $3 where id = $4 RETURNING *",
                    model_record.amount,
                    model_record.record_type,
                    model_record.record_owner,
                    model_record.id,
