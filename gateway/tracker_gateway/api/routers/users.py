...
async def create_user(user: UserCreate) -> UserResponse:
    ...
    try:
        ...
        response = await rabbitmq_client.call(
            ...
            message={
                "command": "create_user",
                "data": {"username": user.username, "email": user.email, "password_hash": password_hash},
            },
            timeout=RPC_TIMEOUT,
        )
...
async def get_user(user_id: int) -> UserResponse:
    ...
    try:
        # Send RPC command to Users service
        response = await rabbitmq_client.call(
            queue_name="users.commands", message={"command": "get_user", "data": {"id": user_id}}, timeout=RPC_TIMEOUT
        )
...
async def list_users(
    ...
    try:
        ...
        response = await rabbitmq_client.call(
            queue_name="users.commands",
            message={"command": "list_users", "data": {"limit": limit, "offset": offset}},
            timeout=RPC_TIMEOUT,
        )
        ...
        return UserListResponse(users=users, total=data.get("total", len(users)), limit=limit, offset=offset)
...
async def update_user(user_id: int, user: UserUpdate) -> UserResponse:
    ...
    try:
        ...
        response = await rabbitmq_client.call(
            queue_name="users.commands",
            message={"command": "update_user", "data": {"id": user_id, "update": update_data}},
            timeout=RPC_TIMEOUT,
        )
...
async def delete_user(user_id: int) -> None:
    ...
    try:
        ...
        response = await rabbitmq_client.call(
            queue_name="users.commands",
            message={"command": "delete_user", "data": {"id": user_id}},
            timeout=RPC_TIMEOUT,
        )
