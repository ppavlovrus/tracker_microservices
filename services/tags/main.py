"""Tags Service - Worker for handling tag commands."""
...
async def handle_command(payload: dict, message: IncomingMessage) -> dict:
    ...
    try:
        if command == "create_tag":
            return await tag_handlers.handle_create_tag(data)
        
        elif command == "get_tag":
            return await tag_handlers.handle_get_tag(data)
        
        elif command == "get_tag_by_name":
            return await tag_handlers.handle_get_tag_by_name(data)
        
        elif command == "update_tag":
            return await tag_handlers.handle_update_tag(data)
        
        elif command == "delete_tag":
            return await tag_handlers.handle_delete_tag(data)
        
        elif command == "list_tags":
            return await tag_handlers.handle_list_tags(data)
...
async def startup():
    ...
    global db_pool, rabbitmq_client, tag_handlers
    ...
    try:
        ...
        tag_repository = TagRepository(db_pool)
        tag_handlers = TagHandlers(tag_repository)
